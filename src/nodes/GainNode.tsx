...
const GainNode = ({ id, data }: any) => {
  const [gain, setGain] = useState(data.gain || 0.5);

  useEffect(() => {
    updateNodeParam(id, 'gain', gain);
  }, [id, gain]);
  ...
  return (
    <div className="bg-slate-800 p-4 border border-slate-700 rounded-lg shadow-xl min-w-[150px]">
      ...
      <Handle 
        ...
        style={{ top: '70%' }}
      />
      <div className="absolute left-[-30px] top-[65%] text-[8px] text-emerald-400 font-bold uppercase pointer-events-none">Mod</div>
