...
import { getAudioContextState } from '../AudioEngine';
...
const DestinationNode = () => {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setIsActive(getAudioContextState() === 'running');
    }, 500);
  ...
  return (
    <div
      className={`p-4 border-2 rounded-lg transition-all duration-500 min-w-[200px] ${
        isActive
    ...
    >
      ...
      <p className="text-slate-500 text-[10px] leading-tight">
        {isActive ? "El motor d'audio esta ACTIU" : "El motor d'audio esta ATURAT"}
      </p>
