...
const KickSynthNode = ({ id, data, onDataChange }: ControllableSoundNodeProps) => {
  ...
  return (
    <div className="bg-cyan-950/80 backdrop-blur-xl border border-cyan-400/20 p-4 rounded-2xl shadow-2xl min-w-[760px]">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div>
          <div className="text-[10px] font-black tracking-widest text-cyan-300 uppercase mb-1 flex items-center gap-2">
            <div className="w-1.5 h-1.5 bg-cyan-300 rounded-full animate-pulse" />
            Kick Synth
          </div>
          <p className="text-[10px] text-white/40 uppercase tracking-[0.2em]">16-Step Kick Voice</p>
        </div>
          ...
          return (
            <button
              ...
              className={`h-10 rounded-md border text-[9px] font-black transition-all ${
                isActive
                  ? `bg-cyan-300 text-black border-cyan-200 ${isCurrent ? 'shadow-[0_0_18px_rgba(103,232,249,0.8)] scale-[1.03]' : ''}`
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between">
        <span className="text-[8px] uppercase tracking-[0.25em] text-white/35">Kick Out</span>
