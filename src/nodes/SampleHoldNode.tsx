...
const SampleHoldNode = ({ id, data, onDataChange }: ControllableSoundNodeProps) => {
  ...
  return (
    <div className="bg-slate-950/80 backdrop-blur-xl border border-slate-400/20 p-4 rounded-2xl shadow-2xl min-w-[250px]">
      ...
      <div className="flex bg-white/5 rounded-lg p-1 mb-4">
        <button
          ...
          className={`flex-1 text-[8px] py-1 rounded-md transition-all font-bold uppercase ${
            !sync ? 'bg-slate-200 text-black shadow-lg' : 'text-white/40 hover:text-white'
          }`}
        >
          Free
        </button>
        ...
        {sync ? (
          <div className="col-span-2">
            ...
            <select
              ...
              className="bg-slate-900 text-white text-xs p-1.5 rounded border border-slate-400/20 w-full outline-none focus:border-slate-200"
            >
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className="text-[8px] uppercase tracking-[0.25em] text-white/35">Stepped Audio</span>
        </div>
